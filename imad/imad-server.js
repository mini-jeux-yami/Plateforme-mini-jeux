module.exports = function(io) {
    ...
    let gameState = {
        ...
        players: {}, // Indexé par USERNAME maintenant !
        politician: null, 
        logs: [], 
        winners: []
    };
    ...
    function checkVictory() {
        ...
        if (sortedCartels.length > 0 && sortedCartels[0].bank >= WIN_CARTEL) cartelWinner = sortedCartels[0];
        if (gameState.politician.bank >= WIN_POLITICIAN) polWinner = gameState.politician;
    ...
    function resolveTurn() {
            ...
            cartels.forEach(c => {
                ...
                if (actA && actA.type === 'sell' && actA.resource === raidTarget) {
                    ...
                    if (stock > 0) {
                        ...
                        c.inventory[raidTarget] = 0; c.raidedThisTurn = true; 
                        turnLogs.push(`🚨 Le Cartel de ${c.username} a subi une saisie majeure de ${gameState.market[raidTarget].name} !`);
                    }
                }
            });
            if (totalSeizedValue > 0) pol.bank += Math.round(totalSeizedValue * 0.85);
            else turnLogs.push(`🚔 La police a perquisitionné la route des ${gameState.market[raidTarget].name}, mais elle était vide.`);
        }

        // ETAPE 3 : BRAQUAGES
        let protections = {};
        cartels.forEach(c => {
            if (c.currentActions.b && c.currentActions.b.type === 'protect') protections[c.id] = c.currentActions.b.resource;
        });
        cartels.forEach(c => {
            let actB = c.currentActions.b;
            if (actB && actB.type === 'heist') {
                let targetCartel = gameState.players[actB.targetId]; // actB.targetId est le username
                ...
                if (targetCartel) {
                    if (protections[targetCartel.id] === targetRes) {
                        ...
                        if (availableStock > 0) {
                            c.inventory[targetRes] += availableStock; targetCartel.inventory[targetRes] = 0;
        ...
        cartels.forEach(c => {
            ...
            if (actA && actA.type === 'produce') {
                let amount = 1;
                if (c.currentActions.b && c.currentActions.b.type === 'boost') amount = 1.5;
        ...
        cartels.forEach(c => {
            let actA = c.currentActions.a;
            if (actA && actA.type === 'sell' && !c.raidedThisTurn && c.inventory[actA.resource] > 0) sellersCount[actA.resource]++;
        });
        cartels.forEach(c => {
            let actA = c.currentActions.a;
    ...
    function getPublicGameState() {
        let publicPlayers = {};
        for(let uname in gameState.players) {
            let p = gameState.players[uname];
            publicPlayers[uname] = { id: p.id, username: p.username, profile_pic: p.profile_pic, inventory: p.inventory, isReady: p.isReady, isConnected: p.isConnected };
        }
        return {
    ...
    // ==========================================
    // 4. GESTION DES CONNEXIONS ET RECONNEXIONS
    // ==========================================
    imadIo.on('connection', (socket) => {
        ...
        socket.on('join_as_cartel', (user) => {
            if (gameState.status === 'playing') {
                if (gameState.players[user.username]) {
                    gameState.players[user.username].socketId = socket.id;
                    gameState.players[user.username].isConnected = true;
                    imadIo.emit('game_resumed');
                    socket.emit('sync_state', getPublicGameState());
                    return;
                } else return socket.emit('error', 'Une partie est déjà en cours.');
            }

            if (Object.keys(gameState.players).length >= 3 && !gameState.players[user.username]) return socket.emit('error', 'Les 3 Cartels sont complets.');

            gameState.players[user.username] = {
                socketId: socket.id, id: user.username, username: user.username, profile_pic: user.profile_pic,
                bank: 0, inventory: { weapons: 0, contraband: 0, data: 0 },
                currentActions: { a: null, b: null, c: null }, isReady: false, bribeReply: null, isConnected: true
            };
        ...
        socket.on('join_as_politician', (user) => {
            if (gameState.status === 'playing') {
                if (gameState.politician && gameState.politician.username === user.username) {
                    gameState.politician.socketId = socket.id;
                    gameState.politician.isConnected = true;
                    imadIo.emit('game_resumed');
                    socket.emit('sync_state', getPublicGameState());
                    return;
                } else return socket.emit('error', 'Une partie est déjà en cours.');
            ...
            if (gameState.politician && gameState.politician.username !== user.username) return socket.emit('error', 'Le Politicien est déjà pris.');

            gameState.politician = {
                socketId: socket.id, id: user.username, username: user.username, profile_pic: user.profile_pic,
                bank: 0, lastRaidTurn: -99, currentActions: { raid: null, bribesResponse: {} }, isReady: false, isConnected: true
            };
            broadcastLobbyUpdate();
        });
        ...
        function handleDisconnect() {
            ...
            if (gameState.status === 'playing') {
                if (user.role === 'cartel') gameState.players[user.username].isConnected = false;
                if (user.role === 'politician') gameState.politician.isConnected = false;
                imadIo.emit('game_paused', { missingPlayer: user.username });
            } else if (gameState.status === 'lobby') {
                if (user.role === 'cartel') delete gameState.players[user.username];
                if (user.role === 'politician') gameState.politician = null;
                broadcastLobbyUpdate();
            }
        }

        function getUserBySocket(socketId) {
            if (gameState.politician && gameState.politician.socketId === socketId) return { role: 'politician', username: gameState.politician.username };
            for (let uname in gameState.players) {
                if (gameState.players[uname].socketId === socketId) return { role: 'cartel', username: uname };
            }
            return null;
        }
        ...
        function broadcastLobbyUpdate() {
            imadIo.emit('update_lobby', {
                cartels: Object.values(gameState.players).map(p => ({ username: p.username, profile_pic: p.profile_pic, isConnected: p.isConnected })),
                politician: gameState.politician ? { username: gameState.politician.username, profile_pic: gameState.politician.profile_pic, isConnected: gameState.politician.isConnected } : null,
        ...
        socket.on('force_kill_game', () => {
            ...
            gameState.turn = 1;
            gameState.politician = null;
            gameState.players = {};
            imadIo.emit('game_ended_force');
        });
        ...
        socket.on('get_private_info', () => {
            ...
            if (user.role === 'cartel') {
                socket.emit('private_info', { bank: gameState.players[user.username].bank, bribeReply: gameState.players[user.username].bribeReply });
            } else if (user.role === 'politician') {
                let bribes = [];
                Object.values(gameState.players).forEach(c => {
                    if (c.currentActions.c && c.currentActions.c.amount > 0) {
                        bribes.push({ cartelId: c.username, username: c.username, amount: c.currentActions.c.amount, question: c.currentActions.c.question });
                    }
                });
        ...
        socket.on('submit_cartel_actions', (data) => {
            let user = getUserBySocket(socket.id);
            if (user && user.role === 'cartel' && !gameState.players[user.username].isReady) {
                gameState.players[user.username].currentActions = data.actions;
                gameState.players[user.username].isReady = true;
                checkAllReady();
            }
        });
        ...
        socket.on('submit_politician_actions', (actions) => {
            let user = getUserBySocket(socket.id);
            if (user && user.role === 'politician' && !gameState.politician.isReady) {
        ...
        function checkAllReady() {
            ...
            imadIo.emit('turn_start', getPublicGameState());
            if (allCartelsReady && polReady) resolveTurn();
        }
    });
};